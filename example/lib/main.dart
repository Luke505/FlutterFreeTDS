...
class _MyAppState extends State<MyApp> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(title: const Text('FreeTDS example')),
        body: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                TextField(decoration: const InputDecoration(labelText: 'Hostname'), controller: hostname, enabled: !loading),
                TextField(decoration: const InputDecoration(labelText: 'Username'), controller: username, enabled: !loading),
                TextField(decoration: const InputDecoration(labelText: 'Password'), controller: password, obscureText: true, enabled: !loading),
                TextField(decoration: const InputDecoration(labelText: 'Database'), controller: database, enabled: !loading),
                DropdownButtonFormField<String>(
                  initialValue: encryption.text,
                  icon: const Icon(Icons.keyboard_arrow_down),
                  //elevation: 16,
                  style: const TextStyle(color: Colors.black),
                  decoration: const InputDecoration(enabledBorder: InputBorder.none, labelText: "Encryption"),
                  ...
                  items:
                      [...SYBEncryptionLevel.values].map<DropdownMenuItem<String>>((item) {
                        return DropdownMenuItem<String>(value: item.value, child: Text(item.name, style: const TextStyle(color: Colors.black, fontSize: 18.0)));
                      }).toList(),
                ),
                TextField(
                ...
                Center(child: Padding(padding: const EdgeInsets.all(16.0), child: ElevatedButton(onPressed: loading ? null : execute, child: const Text('Execute')))),
                if (error != null) Center(child: Text("Error: $error")),
                if (tables != null)
                  Column(
                    children: [
                      Center(child: Column(children: [Text("Affected rows: ${getResultAffectedRows(tables!)}"), Text("# Rows: ${getResultRowCounts(tables!)}")])),
                      ...(tables!
                          .map(
                            (table) =>
                                    ...
                                    ? Padding(
                                      ...
                                      child: SingleChildScrollView(
                                        ...
                                        child: DataTable(
                                          ...
                                          columns: [
                                            const DataColumn(label: Expanded(child: Text("#", style: TextStyle(fontWeight: FontWeight.bold))), numeric: true),
                                            ...table.columns.map((column) => DataColumn(label: Expanded(child: Text(column, style: const TextStyle(fontWeight: FontWeight.bold))))),
                                          ],
                                          rows:
                                              table.data
                                                  .mapIndexed(
                                                    (i, row) => DataRow(
                                                      cells: [
                                                        DataCell(Text((i + 1).toString()), placeholder: true),
                                                        ...table.columns.map((column) => DataCell(Text(row[column]?.toString() ?? "NULL"), placeholder: row[column] == null)),
                                                      ],
                                                    ),
                                                  )
                                                  .toList(),
  ...
  Future<void> execute() async {
    ...
    try {
      ...
      try {
        await FreeTDS.connect(host: hostname.text, username: username.text, password: password.text, database: database, encryption: encryption);
      ...
      try {
        tmpResult = await FreeTDS.query(query.text, []);
      } on FreeTDSException catch (e, st) {
      ...
      for (var table in tmpResult) {
        logger.i(
          "output of #${table.affectedRows} affected rows: ${json.encode(table.data, toEncodable: (dynamic object) {
            try {
