...
class LibraryQueryParam {
  ...
  LibraryQueryParam(dynamic value, {this.name, this.output = 0, this.datatype = 0, this.maxlen = 0, this.scale, this.precision, this.datalen = 0}) {
    if (value == null) {
      _nullValue();
  ...
  LibraryQueryParam.fromQueryParam(QueryParam param) {
    ...
    if (param.value == null) {
      ...
      _stringValue(param.value);
    } else if (param.value is int) {
      _intValue(BigInt.from(param.value));
    } else if (param.value is BigInt) {
      _intValue(param.value);
    } else if (param.value is double) {
      _doubleValue(Decimal.parse((param.value as double).toStringAsFixed(20)));
    } else if (param.value is Decimal) {
      _doubleValue(param.value);
    } else if (param.value is Uint8List || param.value is Int8List) {
      _uInt8ListValue(param.value);
    } else if (param.value is LocalDateTime || param.value is LocalDate || param.value is LocalTime || param.value is OffsetDateTime) {
      _stringValue(param.value.toString());
    } else if (param.value is bool) {
      _uInt8ListValue(Uint8List.fromList([param.value ? 1 : 0]));
    } else {
      throw UnsupportedError("Unsupported type: ${param.value.runtimeType}");
    }
  }

  LibraryQueryParam.custom({
    this.name,
    this.output = 0,
    this.datatype = 0,
    this.maxlen = 0,
  ...
  LibraryQueryParam.nullValue({this.name, this.output = 0, this.datatype = 0, this.maxlen = 0, this.scale = 0, this.precision = 0, this.datalen = 0}) {
    _nullValue();
  }
  ...
  @override
  String toString() => "LibraryQueryParam{name: $name, output: $output, datatype: $datatype, maxlen: $maxlen, scale: $scale, precision: $precision, datalen: $datalen}";
}
